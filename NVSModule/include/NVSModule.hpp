...
namespace NVSModule
{
    ...
    public:
        NVSStorage();
        ~NVSStorage();
        esp_err_t ReadTimeJson(char **data);
        ...
        esp_err_t read_connectors_offline_data(uint8_t ConnId, char *data);
    ...
    private:
        esp_err_t readFile(const char *filename, char *data);
        esp_err_t readFile_(const char *filename, char **data);
        esp_err_t writeFile(const char *filename, const char *data);
    };
} // namespace NVSModule
